...
namespace GraphTest.Schedulers
{
    ...
    /// <summary>
    /// This scheduler finds the most optimal schedule using microsoft solver foundation
    /// </summary>
    class ILP
    {
        TaskGraph graph;
        ...
        public void Solve()
        {
            ...
            Link[] dependencies = new Link[nNodes*nNodes];
            
            foreach (var node in graph.Nodes) {
                nodes.Add(new Node(node.ID-1, node.SimulatedExecutionTime));
                var id = node.ID-1;
                foreach (var node2 in graph.Nodes) {
                    ...
                    if (node.ParentNodes.Contains(node2)) {
                        dependencies[id * nNodes + id2] = new Link(id, id2, 1);
                    } else {
                        dependencies[id * nNodes + id2] = new Link(id, id2, 0);
                    }
                }
            }

            List<SolverWorker> workers = new List<SolverWorker>() {
                            new SolverWorker(),
                            new SolverWorker()
                        };

            FindSolution(nodes, dependencies.AsEnumerable(), workers);
